...
// ✅ Use the same output folder as in `playwright.config.ts`
...
export default async function globalTeardown() {
  console.log(`📢 Generating Allure Report from ${resultsDir}... Open it using: allure open ${reportDir}`);
  try {
    ...
    // ✅ Step 3: Write back the modified JSON
    fs.writeFileSync(summaryFile, JSON.stringify(summaryData, null, 2));
    console.log(`✅ Allure Report generated successfully! Open it using: allure open ${reportDir}`);
  } catch (error) {
